# Hebrew Signs — Dynamic Interlinear: Deuteronomy 6:4-9

**Generated:** 2026-03-04
**Framework:** Corrected 22-Letter Dynamics
**Words:** 48 | **Verses:** 6

## How to Read

Each verse is presented word by word with three layers:

1. **Hebrew** — the pointed (vocalized) text from the Leningrad Codex
2. **Meaning** — traditional English meaning of the full word
3. **Dynamic** — what the word DOES according to its letter dynamics

In the Root column: prefixes are shown before **+**, the **root** is bolded, and suffixes follow after **+**.

Prefix dynamics: ב=Containment, ה=Revelation, ו=Conjunction, כ=Actualization, ל=Direction, מ=Flow, ש=Intensification.

Root dynamics read as a micro-sentence: R1 (initiates) → R2 (mediates) → R3 (completes).

**Verb Stems** (binyanim) — detected via MorphHB morphological tagging:

| Label | Stem | Meaning Shift |
|-------|------|---------------|
| *(unmarked)* | Qal | Base/simple action |
| [received/reflected] | Niphal | Passive or reflexive |
| [intensive] | Piel | Intensive or factitive |
| [intensive-received] | Pual | Intensive passive |
| [causative] | Hiphil | Causative active |
| [causative-received] | Hophal | Causative passive |
| [reflexive] | Hitpael | Reflexive or reciprocal |

---

### Deuteronomy 6:4

> שְׁמַ֖ע יִשְׂרָאֵ֑ל יְהוָ֥ה אֱלֹהֵ֖ינוּ יְהוָ֥ה ׀ אֶחָֽד׃
>
> *Hear! Israel YHWH our God YHWH one*

| # | Hebrew | Meaning | Root | Dynamic |
|---|--------|---------|------|---------|
| 1 | שְׁמַ֖ע | Hear! | **שמע** | Intensification → Flow → Perception |
| 2 | יִשְׂרָאֵ֑ל | Israel | **ישר** + אל | Agency → Intensification → Headship (suffix) |
| 3 | יְהוָ֥ה | YHWH | **יהו** + ה | Agency → Revelation → Conjunction (feminine/directional) |
| 4 | אֱלֹהֵ֖ינוּ | our God | **אלה** + ינו | Origination → Direction → Revelation (us/our) |
| 5 | יְהוָ֥ה ׀ | YHWH | **יהו** + ה | Agency → Revelation → Conjunction (feminine/directional) |
| 6 | אֶחָֽד׃ | one | **אחד** | Origination → Vitalization → Passage |

### Deuteronomy 6:5

> וְאָ֣הַבְתָּ֔ אֵ֖ת יְהוָ֣ה אֱלֹהֶ֑יךָ בְּכָל־ לְבָבְךָ֥ וּבְכָל־ נַפְשְׁךָ֖ וּבְכָל־ מְאֹדֶֽךָ׃
>
> *and you shall love direct object marker the LORD your God with all your heart and with all your soul and with all your very self*

| # | Hebrew | Meaning | Root | Dynamic |
|---|--------|---------|------|---------|
| 1 | וְאָ֣הַבְתָּ֔ | and you shall love | ו + **אהב** + ת | Conjunction + Origination → Revelation → Containment (feminine/construct) |
| 2 | אֵ֖ת | direct object marker | את | Origination → Completion |
| 3 | יְהוָ֣ה | the LORD | **יהו** + ה | Agency → Revelation → Conjunction (feminine/directional) |
| 4 | אֱלֹהֶ֑יךָ | your God | **אלה** + יך | Origination → Direction → Revelation (your (pl)) |
| 5 | בְּכָל־ | with all | **בכל** | Containment → Actualization → Direction |
| 6 | לְבָבְךָ֥ | your heart | ל + **בבך** | Direction + Containment → Containment → Actualization |
| 7 | וּבְכָל־ | and with all | ו + **בכל** | Conjunction + Containment → Actualization → Direction |
| 8 | נַפְשְׁךָ֖ | your soul | **נפש** + ך | Propagation → Projection → Intensification (your (m.sg)) |
| 9 | וּבְכָל־ | and with all | ו + **בכל** | Conjunction + Containment → Actualization → Direction |
| 10 | מְאֹדֶֽךָ׃ | your very self | **מאד** + ך | Flow → Origination → Passage (your (m.sg)) |

### Deuteronomy 6:6

> וְהָי֞וּ הַדְּבָרִ֣ים הָאֵ֗לֶּה אֲשֶׁ֨ר אָנֹכִ֧י מְצַוְּךָ֛ הַיּ֖וֹם עַל־ לְבָבֶֽךָ׃
>
> *and they will be the words/matters these these (things) which/that I (emphatic) I command you the day/today upon/over your heart*

| # | Hebrew | Meaning | Root | Dynamic |
|---|--------|---------|------|---------|
| 1 | וְהָי֞וּ | and they will be | ו + **היו** | Conjunction + Revelation → Agency → Conjunction |
| 2 | הַדְּבָרִ֣ים | the words/matters these | ה + **דבר** + ים | Revelation + Passage → Containment → Headship (masculine plural) |
| 3 | הָאֵ֗לֶּה | these (things) | ה + **אלה** | Revelation + Origination → Direction → Revelation |
| 4 | אֲשֶׁ֨ר | which/that | **אשר** | Origination → Intensification → Headship |
| 5 | אָנֹכִ֧י | I (emphatic) | **אנכ** + י | Origination → Propagation → Actualization (my/of) |
| 6 | מְצַוְּךָ֛ | I command you | מ + **צוך** | Flow + [intensive] Alignment → Conjunction → Actualization |
| 7 | הַיּ֖וֹם | the day/today | ה + **יום** | Revelation + Agency → Conjunction → Flow |
| 8 | עַל־ | upon/over | על | Perception → Direction |
| 9 | לְבָבֶֽךָ׃ | your heart | ל + **בבך** | Direction + Containment → Containment → Actualization |

### Deuteronomy 6:7

> וְשִׁנַּנְתָּ֣ם לְבָנֶ֔יךָ וְדִבַּרְתָּ֖ בָּ֑ם בְּשִׁבְתְּךָ֤ בְּבֵיתֶ֙ךָ֙ וּבְלֶכְתְּךָ֣ בַדֶּ֔רֶךְ וּֽבְשָׁכְבְּךָ֖ וּבְקוּמֶֽךָ׃
>
> *and you shall impress them to your sons and you shall speak in them when you sit in your house and when you walk on the way and when you lie down and when you rise*

| # | Hebrew | Meaning | Root | Dynamic |
|---|--------|---------|------|---------|
| 1 | וְשִׁנַּנְתָּ֣ם | and you shall impress them | **ושנ** + נתם | [intensive] Conjunction → Intensification → Propagation (suffix) |
| 2 | לְבָנֶ֔יךָ | to your sons | לב + **ניך** | Direction+Containment + Propagation → Agency → Actualization |
| 3 | וְדִבַּרְתָּ֖ | and you shall speak | ו + **דבר** + ת | Conjunction + [intensive] Passage → Containment → Headship (feminine/construct) |
| 4 | בָּ֑ם | in them | בם | Containment → Flow |
| 5 | בְּשִׁבְתְּךָ֤ | when you sit | בש + **בתך** | Containment+Intensification + Containment → Completion → Actualization |
| 6 | בְּבֵיתֶ֙ךָ֙ | in your house | בב + **יתך** | Containment+Containment + Agency → Completion → Actualization |
| 7 | וּבְלֶכְתְּךָ֣ | and when you walk | וב + **לכת** + ך | Conjunction+Containment + Direction → Actualization → Completion (your (m.sg)) |
| 8 | בַדֶּ֔רֶךְ | on the way | ב + **דרך** | Containment + Passage → Headship → Actualization |
| 9 | וּֽבְשָׁכְבְּךָ֖ | and when you lie down | וב + **שכב** + ך | Conjunction+Containment + Intensification → Actualization → Containment (your (m.sg)) |
| 10 | וּבְקוּמֶֽךָ׃ | and when you rise | וב + **קומ** + ך | Conjunction+Containment + Summons → Conjunction → Flow (your (m.sg)) |

### Deuteronomy 6:8

> וּקְשַׁרְתָּ֥ם לְא֖וֹת עַל־ יָדֶ֑ךָ וְהָי֥וּ לְטֹטָפֹ֖ת בֵּ֥ין עֵינֶֽיךָ׃
>
> *and you shall bind them for a sign on your hand and they shall be for frontlets between your eyes*

| # | Hebrew | Meaning | Root | Dynamic |
|---|--------|---------|------|---------|
| 1 | וּקְשַׁרְתָּ֥ם | and you shall bind them | ו + **קשר** + תם | Conjunction + Summons → Intensification → Headship (suffix) |
| 2 | לְא֖וֹת | for a sign | ל + **אות** | Direction + Origination → Conjunction → Completion |
| 3 | עַל־ | on | על | Perception → Direction |
| 4 | יָדֶ֑ךָ | your hand | **ידך** | Agency → Passage → Actualization |
| 5 | וְהָי֥וּ | and they shall be | ו + **היו** | Conjunction + Revelation → Agency → Conjunction |
| 6 | לְטֹטָפֹ֖ת | for frontlets | ל + **טטפ** + ת | Direction + Materialization → Materialization → Projection (feminine/construct) |
| 7 | בֵּ֥ין | between | **בין** | Containment → Agency → Propagation |
| 8 | עֵינֶֽיךָ׃ | your eyes | **עינ** + יך | Perception → Agency → Propagation (your (pl)) |

### Deuteronomy 6:9

> וּכְתַבְתָּ֛ם עַל־ מְזוּזֹ֥ת בֵּיתֶ֖ךָ וּבִשְׁעָרֶֽיךָ׃
>
> *and you shall write them — doorposts your house and in your gates*

| # | Hebrew | Meaning | Root | Dynamic |
|---|--------|---------|------|---------|
| 1 | וּכְתַבְתָּ֛ם | and you shall write them | ו + **כתב** + תם | Conjunction + Actualization → Completion → Containment (suffix) |
| 2 | עַל־ | — | על | Perception → Direction |
| 3 | מְזוּזֹ֥ת | doorposts | מ + **זוז** + ת | Flow + Division → Conjunction → Division (feminine/construct) |
| 4 | בֵּיתֶ֖ךָ | your house | ב + **יתך** | Containment + Agency → Completion → Actualization |
| 5 | וּבִשְׁעָרֶֽיךָ׃ | and in your gates | וב + **שער** + יך | Conjunction+Containment + Intensification → Perception → Headship (your (pl)) |

---

## Notes

### Recurring Patterns

| Dynamic Pattern | Occurrences | Words |
|-----------------|-------------|-------|
| Agency → Revelation → Conjunction | 3 | YHWH, the LORD |
| Origination → Direction → Revelation | 3 | our God, your God, these (things) |
| Containment → Actualization → Direction | 3 | with all, and with all |

### Key Observations

1. **יהי / ויהי** ("let there be" / "and it was") — Agency → Revelation → Agency. The act of creation is an act that reveals and acts again. The hand makes visible, the hand completes.
2. **אור** ("light") — Origination → Conjunction → Headship. Light originates, joins, and leads. It is the first thing to establish presence through connection.
3. **אלהים** ("God") — Origination → Direction → Revelation. The divine name dynamically reads: that which originates, directs toward, and makes visible.
4. **ברא** ("created") — Containment → Headship → Origination. Creation is enclosing that leads toward establishing new presence. Form given to lead into being.
5. **טוב** ("good") — Materialization → Conjunction → Containment. Goodness is what emerges physically, joins together, and takes form. Wholeness materialized.
6. **ערב / בקר** ("evening / morning") — Perception→Headship→Containment / Containment→Summons→Headship. Evening: seeing that leads into enclosure. Morning: from enclosure, a summons that leads forward.
7. **את** (object marker) — Origination → Completion. Aleph to Tav, first to last. The object marker dynamically spans the entire alphabet.

### Known Limitations

- **שמים** ("heavens") is decomposed as ש+מים (Intensification + root מים). The root extraction treats ש as a prefix. The actual etymology of שמים is debated; this decomposition is heuristic.
- **Verb stems** (binyanim) are detected via MorphHB morphological tagging. Non-Qal verbs show a stem label (e.g., [causative] for Hiphil) before the root dynamics.
- **Suffix labeling** is approximate. Some multi-letter suffixes are labeled generically.
- This is a **raw dynamic reading**, not a polished translation. The dynamics show structural relationships; readable English rendering is a separate step.

---

## Source

**Corpus:** UXLC Leningrad Codex
**Framework:** [Corrected 22-Letter Dynamics](../framework/corrected-framework.md)
**Tool:** `scripts/glosser.js`
**Data:** `data/glosser-output/`
